...
class BAIRDataset(Dataset):
    ...
    def __init__(self, data_path, frames_per_sample=5, frames_pred=5, tot_len=15, random_time=True, random_horizontal_flip=True, color_jitter=0,
        ...
        path = os.path.join(self.data_path,'1.npz')
        if os.path.exists(path):
            self.videos = np.load(path)['arr_0']
        else:
            self.videos=[]
            for index in range(len(self.videos_ds)):
            ...
            np.savez(os.path.join(self.data_path,'1.npz'),arr)
    ...
    def __getitem__(self, index, time_idx=0):
        ...
        if self.tot_len>self.frames_pred:
            ...
            img0=transforms.RandomHorizontalFlip(flip_p)(transforms.ToTensor()(img0))
            time_idx+=np.random.randint(0,min(self.tot_len,video_len-self.frames_per_sample)//self.frames_pred)*self.frames_pred
        else:
            if self.random_time and video_len > self.frames_per_sample:
                time_idx = np.random.choice(video_len - self.frames_per_sample)
            img0=self.videos[index][time_idx]
        ...
        # print(time_idx)
        # print(video_len)
        # print(self.frames_per_sample)

        if self.with_target:
            return img0,data, torch.tensor(1)
        else:
            return img0,data
