...
def get_dataset(data_path, config, video_frames_pred=0, start_at=0):
    ...
    if config.data.dataset.upper() == 'CIFAR10':
        ...
        dataset = BAIRDataset(os.path.join(data_path, "train"), frames_per_sample=frames_per_sample, frames_pred=config.data.num_frames,
                               tot_len=config.sampling.num_frames_pred+config.data.num_frames_cond, random_time=True,
                              random_horizontal_flip=config.data.random_flip, color_jitter=getattr(config.data, 'color_jitter', 0.0))
        test_dataset = BAIRDataset(os.path.join(data_path, "test"), frames_per_sample=frames_per_sample,frames_pred=config.data.num_frames,
                                     tot_len=-1, random_time=True,
        ...
        # |-- shard_0001.hdf5
        frames_per_sample = config.data.num_frames_cond + getattr(config.data, "num_frames_future", 0) + video_frames_pred
        dataset = UCF101Dataset(data_path, frames_per_sample=frames_per_sample, frames_pred=config.data.num_frames,
                               tot_len=config.sampling.num_frames_pred+config.data.num_frames_cond, image_size=config.data.image_size, train=True, random_time=True,
                                random_horizontal_flip=config.data.random_flip)
        test_dataset = UCF101Dataset(data_path, frames_per_sample=frames_per_sample,frames_pred=config.data.num_frames,
                                     tot_len=-1, image_size=config.data.image_size, train=False, random_time=True,
