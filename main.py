...
def parse_args_and_config():
    ...
    parser.add_argument('--test_speed', action='store_true', help='Whether to calculate flops')
    parser.add_argument('-s', '--speed_folder', type=str, default='speed', help="The folder name of speed data")
    ...
    if not args.test and not args.sample and not args.video_gen and not args.fast_fid and not args.test_speed:
        if not args.resume_training:
            if os.path.exists(args.log_path):
                ...
                if overwrite:
                    # assert 0,'overwriting existing training records!'
                    # shutil.copytree(args.log_path,'../output/backup')
        ...
        if args.sample:
            ...
            # if new_config.sampling.final_only:
            os.makedirs(os.path.join(args.exp, 'speed'), exist_ok=True)
            args.speed_folder = os.path.join(args.exp, args.speed_folder)
            # else:
            #     os.makedirs(os.path.join(args.exp, f'image_samples_{new_config.sampling.ckpt_id}'), exist_ok=True)
            #     args.image_folder = os.path.join(args.exp, f'image_samples_{new_config.sampling.ckpt_id}', args.image_folder)

            if not os.path.exists(args.speed_folder):
                os.makedirs(args.speed_folder)
            else:
                overwrite = False
                if args.ni:
                    overwrite = True
                else:
                    response = input(f"Video folder {args.speed_folder} already exists.\nOverwrite? (Y/N)")
                    if response.upper() == 'Y':
                        overwrite = True

                if overwrite:
                    shutil.rmtree(args.speed_folder)
                    os.makedirs(args.speed_folder)
                else:
                    print("Output video folder exists. Program halted.")
                    sys.exit(0)

            with open(os.path.join(args.speed_folder, 'config.yml'), 'w') as f:
                yaml.dump(config, f, default_flow_style=False)

            with open(os.path.join(args.speed_folder, 'args.yml'), 'w') as f:
                yaml.dump(vars(args), f, default_flow_style=False)
