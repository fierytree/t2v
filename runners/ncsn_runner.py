...
class NCSNRunner():
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for batch, (x0, X, y) in enumerate(dataloader):
                # print(x0.shape)
                # print(X.shape)
                ...
                # import torchvision
                # print(X.shape)
                # torchvision.utils.save_image(X[:,0,...],'1.png')
                # assert(0)
                ...
                # Loss
                itr_start = time.time()
                loss = anneal_dsm_score_estimation(scorenet, x0, X, labels=None, cond=cond, cond_mask=cond_mask,
                ...
                if step == 1 or step % getattr(self.config.training, "log_freq", 1) == 0:
                    ...
                    print("elapsed: {}, train time: {:.04f}, mem: {:.03f}GB, GPUmem: {:.03f}GB, step: {}, lr: {:.06f}, grad: {:.04f}, loss: {:.04f}".format(
                        str(datetime.timedelta(seconds=(time.time() - self.start_time)) + datetime.timedelta(seconds=self.time_elapsed_prev*3600))[:-3],
                        self.time_train.val, get_proc_mem(), get_GPU_mem(), step, lr, grad_norm, loss.item()))
                if self.config.model.ema:
                ...
                if step == 1 or step % self.config.training.val_freq == 0:
                    try:
                        test_x0, test_X, test_y = next(test_iter)
                    ...
                    with torch.no_grad():
                        test_dsm_loss = anneal_dsm_score_estimation(test_scorenet, test_x0, test_X, labels=None, cond=test_cond, cond_mask=test_cond_mask,
                                                                    loss_type=getattr(self.config.training, 'loss_type', 'a'),
                                                                    gamma=getattr(self.config.model, 'gamma', False),
                    ...
                    print("elapsed: {}, step: {}, mem: {:.03f}GB, GPUmem: {:.03f}GB, test_loss: {:.04f}".format(
                        str(datetime.timedelta(seconds=(time.time() - self.start_time)) + datetime.timedelta(seconds=self.time_elapsed_prev*3600))[:-3],
                        step, get_proc_mem(), get_GPU_mem(), test_dsm_loss.item()))
                ...
                if (step % self.config.training.snapshot_freq == 0 or step % self.config.training.sample_freq == 0) and self.config.training.snapshot_sampling:
                    ...
                    if conditional:
                        ...
                        test_x0 = test_x0[:len(init_samples)].to(self.config.device)
                    ...
                    all_samples = sampler(init_samples, test_scorenet, x0=test_x0, cond=test_cond, cond_mask=test_cond_mask,
    ...
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        def my_collate(batch):
            ...
            x0 = torch.stack(x0).repeat_interleave(preds_per_test, dim=0)
            return x0, data, torch.zeros(len(data))
        ...
        for i, (x0, real_, _) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)), desc="\nvideo_gen dataloader"):
            ...
            print(f"(1) Video {'Pred' if future == 0 else 'Interp'}")
            ...
            if self.config.sampling.data_init:
                try:
                    x0, real_init, _ = next(data_iter2)
                except StopIteration:
                    if self.config.data.dataset.upper() == 'FFHQ':
            ...
            for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                ...
                gen_samples = sampler(init_samples if i_frame==0 or getattr(self.config.sampling, 'init_prev_t', -1) <= 0 else gen_samples,
                                      mynet,x0 = x0, cond=cond, cond_mask=cond_mask,
            ...
            if future > 0 and prob_mask_future > 0.0 and not self.prob_mask_sync:
                ...
                print(f"(2) Video Pred")

                num_frames_pred = self.config.sampling.num_frames_pred

                logging.info(f"PREDICTING {num_frames_pred} frames, using a {self.config.data.num_frames} frame model conditioned on {self.config.data.num_frames_cond} frames, subsample={getattr(self.config.sampling, 'subsample', None)}, preds_per_test={preds_per_test}")
                print(f"PREDICTING {num_frames_pred} frames, using a {self.config.data.num_frames} frame model conditioned on {self.config.data.num_frames_cond} frames, subsample={getattr(self.config.sampling, 'subsample', None)}, preds_per_test={preds_per_test}")
            ...
            print(f"fvd1 {calc_fvd1}, fvd2 {calc_fvd2}, fvd3 {calc_fvd3}")
            pred_uncond = None
            if calc_fvd1 or calc_fvd2 or calc_fvd3:
                ...
                if calc_fvd3:
                    ...
                    print(f"(3) Video Gen - Uncond - FVD")
                    ...
                    print(f"GENERATING (Uncond) {num_frames_pred} frames, using a {self.config.data.num_frames} frame model (conditioned on {self.config.data.num_frames_cond} cond + {self.config.data.num_frames_future} futr frames), subsample={getattr(self.config.sampling, 'subsample', None)}, preds_per_test={preds_per_test}")
                    ...
                    for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                        ...
                        # Generate samples
                        gen_samples = sampler(init_samples if i_frame==0 or getattr(self.config.sampling, 'init_prev_t', -1) <= 0 else gen_samples,
                                              scorenet, x0=x0, cond=cond_fvd, cond_mask=cond_mask_fvd,
                                              n_steps_each=self.config.sampling.n_steps_each, step_lr=self.config.sampling.step_lr,
                                              verbose=True if not train else False, final_only=True, denoise=self.config.sampling.denoise,
                                              subsample_steps=getattr(self.config.sampling, 'subsample', None),
                                              clip_before=getattr(self.config.sampling, 'clip_before', True),
        ...
        if train:
            ...
            logging.info(f"elapsed: {elapsed}, {format_p(vid_metrics)}")
            print(f"elapsed: {elapsed}, {format_p(vid_metrics)}")
            self.write_to_yaml(os.path.join(self.args.video_folder, 'vid_metrics.yml'), vid_metrics)

    @torch.no_grad()    
    def test_speed(self, scorenet=None, train=False):
        speed_metrics={}
        # Conditional
        conditional = self.config.data.num_frames_cond > 0
        assert conditional, f"Video generating model has to be conditional! num_frames_cond has to be > 0! Given {self.config.data.num_frames_cond}"
        ...
        if scorenet is None:
            ckpt_path=os.path.join(self.args.log_path, 'checkpoint.pt')
            scorenet = get_model(self.config)
        ...
        from models.better.layerspp import ResnetBlockBigGANppGN, AttnBlockpp
        from models.better.fast_attention import SelfAttention
        scorenet=scorenet.to(self.config.device)
        N_blocks=len(scorenet.unet.all_modules)
        attn=[]
        conv=[]
        for i in range(N_blocks):
            if type(scorenet.unet.all_modules[i])==ResnetBlockBigGANppGN:
                conv.append(i)
            elif type(scorenet.unet.all_modules[i]) in [SelfAttention,AttnBlockpp]:
                attn.append(i)
        
        L = len(self.config.model.ch_mult)
        num_res_blocks = self.config.model.num_res_blocks
        all_resolutions = [self.config.data.image_size // (2 ** i) for i in range(L)]
        attn_resolutions = self.config.model.attn_resolutions
        nf = self.config.model.ngf
        temb_dim = nf * 4
        if getattr(self.config.model, 'cond_emb', False):
            temb_dim += nf // 2

        cnt=min(conv)
        layers=[[] for i in range(L)]
        level=-1*np.ones(N_blocks,dtype=int)

        def add(cnt,lst):
            assert(cnt in lst), print(scorenet,file=open('1.txt','w'))
            layers[i_level].append(cnt)
            level[cnt]=i_level
            return cnt+1 
        ...
        for i_level in reversed(range(L)):
        ...
        # N_blocks=41
        # attn=[7,9,12,14,17,19,21,26,31,36]
        # layers=[
        #     [3,4,5,38,39,40],
        ...
        for i in range(N_blocks):
            params=count_parameters(scorenet.unet.all_modules[i])
            idx1=level[i]
            idx2=int(i in attn)            
            if idx1 !=-1:
                results[idx1][idx2]+=params
            # print(idx1,idx2,params)
        print(results/1e6)
        for i in range(L):            
            speed_metrics[f'Mparams_conv_layer{i+1}']=float(results[i][0]/1e6)
            speed_metrics[f'Mparams_attn_layer{i+1}']=float(results[i][1]/1e6)
        
        # inference time
        N=300
        im_size=self.config.data.image_size
        t = torch.zeros(1,dtype=torch.float32).to(self.config.device)
        ...
        x, cond, cond_mask = conditioning_fn(self.config, X, num_frames_pred=self.config.data.num_frames,
                                prob_mask_cond=prob_mask_cond,prob_mask_future=prob_mask_future,conditional=conditional)

        flops_dict,_ = flop_count(scorenet,(x,t,cond))
        flops = FlopCountAnalysis(scorenet,(x,t,cond))
        ...
        assert(L>=1)
        bs,ch,h=self.config.training.batch_size,self.config.model.ngf,all_resolutions[1]
        x = torch.randn(bs,ch,h,h).to(self.config.device)
        temb = torch.randn(1,temb_dim).to(self.config.device)
        net = scorenet.unet.all_modules[layers[1][0]]
        ...
        # print(flops_dict)
        dic=flops.by_module()
        dic2={'layer1':{},'layer2':{},'layer3':{},'layer4':{}}
        for i in range(N_blocks):
            i_level=level[i]
            if i_level==-1:
                continue
            t='attn' if i in attn else 'conv'
            dic2[f'layer{i_level+1}']['({:02d})'.format(i)]={'_type':t,
                    'GFLOPs':dic[f'unet.all_modules.{i}']/1e9,'modules_GFLOPs':{}}
        ...
        for item in dic:
            ...
            i_level=level[i]
            if i_level!=-1:
                dic2[f'layer{i_level+1}']['({:02d})'.format(i)]['modules_GFLOPs'][item]=dic[item]/1e9
        self.write_to_yaml(os.path.join(self.args.speed_folder, 'module_flops.yml'), dic2)

        results2=np.zeros((L,2))
        for i in range(N_blocks):
            s='unet.all_modules.'+str(i)
            flops=dic[s]
            idx1=level[i]
            idx2=int(i in attn)
            if idx1 !=-1:
                results2[idx1][idx2]+=flops
        print(results2/1e9)
        for i in range(L):            
            speed_metrics[f'GFLOPs_conv_layer{i+1}']=float(results2[i][0]/1e9)
            speed_metrics[f'GFLOPs_attn_layer{i+1}']=float(results2[i][1]/1e9)

        mean_syn = np.sum(timings) / N
        std_syn = np.std(timings)
        mean_syn2 = np.sum(timings2) / N
        std_syn2 = np.std(timings2)
        flops=sum(list(flops_dict.values()))
        speed_metrics['GFLOPs']=flops
        speed_metrics['time_mean']=float(mean_syn)
        speed_metrics['time_std']=float(std_syn)
        speed_metrics['time_conv_mean']=float(mean_syn2)
        speed_metrics['time_conv_std']=float(std_syn2)
        speed_metrics['params']=count_parameters(scorenet)/1e6
        logging.info("flops_dict: " +str(flops_dict))
        logging.info("flops: %.2fG params: %.2fM time:%.2f +- %.2fms, %.2f +- %.2fms" % 
                (flops, count_parameters(scorenet)/1e6,mean_syn,std_syn,mean_syn2,std_syn2))
        for item in flops_dict:
            speed_metrics[item]=flops_dict[item]

        self.write_to_yaml(os.path.join(self.args.speed_folder, 'speed_metrics.yml'), speed_metrics)
