...
@torch.no_grad()
def ddpm_sampler(x_mod, scorenet, x0=None, cond=None, just_beta=False, final_only=False, denoise=True, subsample_steps=None,
...
def edm_sampler(
    ...
    sigma_min = max(sigma_min, 0.02)
    sigma_max = min(sigma_max, 100)
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        if i < num_steps - 1:
            ...
            denoised = c_skip * x + c_out * F_x.to(torch.float32)
