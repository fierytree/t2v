...
from . import layers, layerspp, fast_attention
...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    fir = True
    fir_kernel = [1, 3, 3, 1]
    self.skip_rescale = skip_rescale = False
    ...
    if self.is3d:
      ...
      AttnBlockDown = AttnBlockUp = functools.partial(fast_attention.SelfAttention, num_heads=1, init_scale=init_scale,
        attention_dropout=0.1, n_head_channels=config.model.n_head_channels,context_size=2*num_frames_cond)
    ...
    self.spynet = SPyNet(pretrained='spynet_20210409-c6c1bd09.pth')
    modules.append(conv3x3(channels*self.n_frames + 2 * self.num_frames_cond, nf))
    hs_c = [nf]
    ...
    for i_level in range(num_resolutions):
      ...
      for i_block in range(num_res_blocks):
        ...
        if all_resolutions[i_level] in attn_resolutions:
          modules.append(AttnBlockDown(hidden_size=in_ch))
        hs_c.append(in_ch)
    ...
    for i_level in reversed(range(num_resolutions)):
      ...
      if all_resolutions[i_level] in attn_resolutions:
        modules.append(AttnBlockUp(hidden_size=in_ch))
  ...
  def forward(self, x, time_cond, x0=None, cond=None, cond_mask=None):
    # timestep/noise_level embedding; only for continuous training
    ...
    x = x.contiguous()
    flow=[]
    for i in range(self.num_frames_cond):
      start=self.channels*(self.num_frames+i)
      end=self.channels*(self.num_frames+i+1)
      xi = x[:,start:end,...]
      flow.append(self.spynet(x0,xi))
    flow = torch.cat(flow,dim=1)
    x = torch.cat([flow,x],dim=1)

    B, C, H, W = flow.shape
    f = flow.reshape(B,C,-1)
    f = f.permute(0,2,1)
...
class SPADE_NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    if self.is3d:
      ...
      #                               init_scale=init_scale,
      #                               skip_rescale=skip_rescale, n_head_channels=config.model.n_head_channels)
      AttnBlock = functools.partial(fast_attention.SelfAttention, num_heads=1, init_scale=init_scale,
        attention_dropout=0.1, n_head_channels=config.model.n_head_channels,context_size=2*num_frames_cond)
    ...
    self.spynet = SPyNet(pretrained='spynet_20210409-c6c1bd09.pth')
    modules.append(conv3x3(channels*self.num_frames+2*self.num_frames_cond, nf))
    hs_c = [nf]
    ...
    for i_level in range(num_resolutions):
      ...
      for i_block in range(num_res_blocks):
        ...
        if all_resolutions[i_level] in attn_resolutions:
          modules.append(AttnBlock(hidden_size=in_ch))
        hs_c.append(in_ch)
...
class UNetMore_DDPM(nn.Module):
  ...
  def forward(self, x, y, x0=None, cond=None, cond_mask=None):
    ...
    return self.unet(x, y, x0, cond, cond_mask=cond_mask)
