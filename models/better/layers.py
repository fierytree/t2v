...
def ddpm_conv1x1(in_planes, out_planes, stride=1, bias=True, init_scale=1., padding=0, groups=1):
  ...
  conv = nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, padding=padding, bias=bias, groups=groups)
...
def ncsn_conv3x3(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1., padding=1, groups=1):
  ...
  conv = nn.Conv2d(in_planes, out_planes, stride=stride, bias=bias,
                   dilation=dilation, padding=padding, kernel_size=3, groups=groups)
  conv.weight.data *= init_scale
  conv.bias.data *= init_scale
  return conv
...
class NIN1d(nn.Module):
  def __init__(self, in_dim, num_units, init_scale=1.0):
    super().__init__()
    self.conv = nn.Conv1d(in_dim, num_units, 1)
...
class Channel_Avg_Pooling(nn.Module):
  def __init__(self, out_ch):
    ...
    self.avg_pooling = nn.AdaptiveAvgPool2d((None,out_ch))
  def forward(self, x):
...
class SpatialWeighting(nn.Module):
  def __init__(self,channels,out_ch,ratio=1):
    super().__init__()
    self.ratio=ratio
    self.global_avgpool = nn.AdaptiveAvgPool2d(1)
    self.conv1=nn.Conv2d(channels, out_ch//ratio, kernel_size=1, stride=1)
    self.act1=nn.ReLU()
    self.conv2=nn.Conv2d(out_ch//ratio, out_ch, kernel_size=1, stride=1)
  ...
  def forward(self, x):
    ...
    B,C,H,W=out.size()
    C1 = C//self.ratio
    out = out.reshape(B*self.ratio,C1,H,W)/C1
    B,C2,H,W=x.size()
    x = x.reshape(B*self.ratio,C2//self.ratio,H,W)
    out = torch.einsum('bchw,bixy->bihw', x, out)
...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.):
        ...
        if dim_head == -1:
            ...
            if query_dim < dim_head:
              _num_heads = 1
            else:
              assert query_dim % dim_head == 0
              _num_heads = query_dim // dim_head
