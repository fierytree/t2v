...
class ResnetBlockBigGANppGN(nn.Module):
  def __init__(self, act, in_ch, out_ch=None, temb_dim=None, up=False, down=False,
    ...
    self.fir_kernel = fir_kernel
    # self.Conv_0 = conv3x3_(in_ch, out_ch)
    self.conv_0_0 = conv1x1(in_ch, out_ch//4)
    self.actnorm2 = get_act_norm(act, act, 'group', out_ch//4, emb_dim=temb_dim, spectral=False, is3d=is3d, n_frames=n_frames)
    self.conv_0_1 = conv3x3(out_ch//4, out_ch//2)
    self.conv_0_2 = conv1x1(out_ch//4, out_ch//2)

    self.actnorm1 = get_act_norm(act, act, 'group', out_ch, emb_dim=temb_dim, spectral=False, is3d=is3d, n_frames=n_frames)
    self.Dropout_0 = nn.Dropout(dropout)
    # self.Conv_1 = conv3x3_(out_ch, out_ch, init_scale=init_scale)
    self.conv_1_0 = conv1x1(out_ch, out_ch//4, init_scale=init_scale)
    self.actnorm3 = get_act_norm(act, act, 'group', out_ch//4, emb_dim=temb_dim, spectral=False, is3d=is3d, n_frames=n_frames)
    self.conv_1_1 = conv3x3(out_ch//4, out_ch//2, init_scale=init_scale)
    self.conv_1_2 = conv1x1(out_ch//4, out_ch//2, init_scale=init_scale)
...
class ResnetBlockBigGANppSPADE(nn.Module):
  def __init__(self, act, in_ch, out_ch=None, temb_dim=None, up=False, down=False, spade_dim=128,
               ...
               skip_rescale=True, init_scale=0., is3d=False, pseudo3d=False, act3d=False, base_ch=1):
    super().__init__()
    ...
    # num_groups = min(out_ch//4 // 4, 32)
    # while (out_ch//4 % num_groups != 0): # must find another value
    #     num_groups -= 1
    self.up = up
    self.down = down
    self.fir = fir
    ...
    # self.Conv_0 = conv3x3_(in_ch, out_ch)
    self.conv_0_0 = conv1x1(in_ch, out_ch//4)
    self.actnorm2 = get_act_norm(act, act, 'spade', out_ch//4, emb_dim=temb_dim, spectral=False, is3d=is3d, n_frames=n_frames,
                                 num_frames_cond=num_frames_cond, cond_ch=cond_ch, spade_dim=spade_dim, cond_conv=conv3x3_, cond_conv1=conv1x1_cond)
    self.conv_0_1 = conv3x3(out_ch//4, out_ch//2)
    self.conv_0_2 = conv1x1(out_ch//4, out_ch//2)
    ...
    # self.Conv_1 = conv3x3_(out_ch, out_ch, init_scale=init_scale)
    self.conv_1_0 = conv1x1(out_ch, out_ch//4, init_scale=init_scale)
    self.actnorm3 = get_act_norm(act, act, 'spade', out_ch//4, emb_dim=temb_dim, spectral=False, is3d=is3d, n_frames=n_frames,
                                 num_frames_cond=num_frames_cond, cond_ch=cond_ch, spade_dim=spade_dim, cond_conv=conv3x3_, cond_conv1=conv1x1_cond)
    self.conv_1_1 = conv3x3(out_ch//4, out_ch//2, init_scale=init_scale)
    self.conv_1_2 = conv1x1(out_ch//4, out_ch//2, init_scale=init_scale)
