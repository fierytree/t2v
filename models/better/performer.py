...
class SLiMPerformer(torch.nn.Module):
  ...
  def __init__(self, vocab_size, hidden_dim, n_layers, ffn_dim, n_heads,
    ...
    self.output_logit_map = NIN(hidden_dim, vocab_size)
...
class SLiMPerformerLayer(torch.nn.Module):
  ...
  def __init__(self, hidden_dim, ffn_dim, n_heads, feature_type, compute_type,
               on_gptln, n_head_channels=-1, init_scale=0.):
    ...
    if n_head_channels == -1:
        ...
        if hidden_dim < n_head_channels:
          _n_heads = 1
        else:
          assert hidden_dim % n_head_channels == 0
          _n_heads = hidden_dim // n_head_channels

    self.attention = MultiHeadAttention(feature_type, _n_heads, hidden_dim,
                                        compute_type)

    self.U_map = NIN(hidden_dim, ffn_dim)
    self.V_map = NIN(ffn_dim, hidden_dim)
...
class MultiHeadAttention(torch.nn.Module):
  ...
  def __init__(self, feature_type, n_heads, hidden_dim, compute_type, init_scale=0.):
    ...
    self.q_map = NIN(hidden_dim, hidden_dim)
    self.k_map = NIN(hidden_dim, hidden_dim)
    self.v_map = NIN(hidden_dim, hidden_dim, init_scale=init_scale)
